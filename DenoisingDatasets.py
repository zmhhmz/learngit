...
from skimage import img_as_float#32 as img_as_float
...
class SimulateTrain(BaseDataSetImg):
    def __init__(self, im_list, length,  pch_size=64, radius=5, noise_type='NIIDGauss',
                                                                        noise_estimate=True, chn=3):
    ...
    def generate_sigma(self):
        ...
        if self.noise_type.lower() == 'niidgauss':
            ...
            sigma_map = np.ones((self.pch_size, self.pch_size),dtype=np.float32) * (sigma/255.0)
    ...
    def __getitem__(self, index):
        ...
        im_ori = cv2.imread(self.im_list[ind_im], 1)/255.0###
        if self.chn == 3:
            im_ori = im_ori[:, :, ::-1]
        elif self.chn == 1:
            im_ori = cv2.cvtColor(im_ori, cv2.COLOR_BGR2GRAY)
            im_ori = im_ori[:, :, np.newaxis]
        im_gt = self.crop_patch(im_ori).astype('float32')
        ...
        if self.noise_estimate:
            ...
            if self.chn == 3:
                sigma2_map_gt = np.tile(sigma2_map_gt, (1, 1, self.chn))
...
class SimulateTest(uData.Dataset):
    ...
    def __getitem__(self, index):
        im_gt = cv2.imread(self.im_list[index], 1)/255.0###
        if self.chn == 3:
            im_gt = im_gt[:, :, ::-1]
        elif self.chn == 1:
        ...
        with h5.File(self.h5_path, 'r') as h5_file:
            noise = np.array(h5_file[im_key][:,:,:self.chn]).astype(np.float32)
        H, W, _ = noise.shape
        im_gt = im_gt[:H, :W].astype(np.float32)
