...
from loss_VAE import loss_fn
from networks import VAENet
...
#    clip_grad_S = args.clip_grad_S
    ...
    param = [x for x in net.named_parameters()]
    for epoch in range(args.epoch_start, args.epochs):
        ...
        for ii, data in enumerate(data_loader[phase]):
            ...
            loss,lossOut,lossX,lossE,lossReY,lossReZ = criterion(Xlist, Zlist,Elist,reYlist,reZlist , im_gt,im_noisy)
            ...
            loss_per_epoch['Loss_Out'] += lossOut.item() / num_iter_epoch[phase]
            loss_per_epoch['Loss_X'] += lossX.item() / num_iter_epoch[phase]
            loss_per_epoch['Loss_E'] += lossE.item() / num_iter_epoch[phase]
            loss_per_epoch['Loss_ReY'] += lossReY.item() / num_iter_epoch[phase]
            loss_per_epoch['Loss_ReZ'] += lossReZ.item() / num_iter_epoch[phase]
            ...
            if (ii+1) % args.print_freq == 0:
                ...
                print(log_str.format(epoch+1, args.epochs, phase, ii+1, num_iter_epoch[phase],
                                         loss.item(), lossOut.item(), lossX.item(),lossE.item(),lossReY.item(),
                                         lossReZ.item(), clip_grad,total_norm, lr))
                ...
                show3 = Xlist[-1].cpu().detach().numpy()[0].transpose((1,2,0))
